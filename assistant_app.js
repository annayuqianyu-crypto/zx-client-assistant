(function () {
  ...
  // S0 KYC 追问库：每问自带贴合的填写示例（大模型会据客户行业进一步定制）
  const PROFILE_GAP_QUESTIONS = {
    subject: '先说说这位客户的基本情况——大致年龄段、在企业或家族里的角色、家庭结构（有没有配偶、子女、二代）？（比如：约55岁，上市公司实控人，已婚，长子已参与家族企业）',
    industry: '客户的主营业务或财富主要来自哪个行业？尽量说到细分赛道和目前所处阶段。（比如：制造业-高端装备、正处产能转型期 / 互联网-SaaS、筹备港股上市）',
    events: '客户近 1-3 年有哪些明确计划或已发生的大事？大致时间点是？（比如：明年 Q2 启动港股上市 / 正在办新加坡身份预计 3 年落地 / 限售股年底解禁后减持）',
    assets: '客户主要资产有哪些类型、大致规模和结构（境内/跨境、流动/非流动）？（比如：A 股股权约 8 亿但限售、境外房产 2 套、可动用现金约 5000 万）',
    constraints: '客户当前主要面临哪些监管、税务或法律约束？（比如：作为实控人受减持新规约束并有信披义务 / 跨境架构涉 CRS 与 37 号文）'
  };
  // S0 缺口追问优先级：主体≈行业≈事件 > 资产 > 约束（前三者缺失最伤下游质量）
  const PROFILE_GAP_PRIORITY = ['subject', 'industry', 'events', 'assets', 'constraints'];
...
现在对话开始，由客户经理先开口。你只需在他说话后，以客户身份回应。`;
  ...
  const CASE_DIRECTIONS = [
    ...
    { icon: '🎯', label: '你想解决客户的什么问题？', hint: '比如', chips: ['CRS境外补税', '公司注销清算', '房产继承过户', '跨境赠与子女', '跨境电商出口'] }
  ];
  // 案例字典右栏：三类可点检索入口（对应线框图「案例字典」）
  const CASE_DICT = [
    { icon: '👤', label: '客户画像', chips: ['制造业老板', '科技新贵', '地产开发商', '跨境贸易商'] },
    { icon: '🏭', label: '客户行业', chips: ['汽配', '跨境电商', '生物医药', '餐饮连锁'] },
    { icon: '⚠️', label: '风险 / 需求分类', chips: ['CRS境外补税', '公司注销清算', '房产继承过户', '跨境赠与子女', '跨境电商出口'] }
  ];
  ...
  // 本地脱敏案例库（源自慧管家真实咨询·已脱敏·选取5个通用低敏感案例内嵌demo；AI 不可用时兜底）
  const CASE_LIBRARY = [
    {
      keys: ['CRS', '境外', '境外资产', '海外账户', '境外收入', '补税', '申报', '香港账户', '信息交换'],
      title: '客户收到税务局境外资产提醒短信，才发现香港理财收益要补税',
      quote: '已经申报过了，怎么还收到提醒？',
      points: ['CRS 报的是账户余额与毛收入，与个人按净收益申报有口径差', '历史年度少报要补税+滞纳金（年化约18.25%）', '目前征税针对境外收益、暂不大规模追溯本金'],
      detail: '背景：客户在香港有逾百万美元存款与稳健理财，收到税务局关于境外资产申报的提醒短信。\n经过：此前已申报仍被提醒，担心本金来源被追溯、又怕高额税负与滞纳金。\n关键点：CRS 每年 8 月交换上一年度数据；征税针对境外收益（利息/分红/赎回收益）而非本金余额；利息股息按 20%。\n我们的做法：向香港分行取 CRS 上报信息表与对账单核对数值，对合理口径差异（毛收入 vs 净收益）主动向税务局解释，历史年度按净收益补申报、尽早处理以止滞纳金。'
    },
    {
      keys: ['注销', '清算', '留抵', '停业', '简易注销', '公司注销', '增值税'],
      title: '商贸公司想干净注销，却卡在一笔大额增值税留抵上',
      quote: '不就是把公司关掉吗，怎么这么难？',
      points: ['商贸行业大额留抵属异常、易触发稽查', '注销前须清理存货/固资/往来款并按(视同)销售缴税', '先工商减资至实缴、剩余财产超实缴部分缴 20% 个税'],
      detail: '背景：某商贸公司业务停滞拟注销。\n经过：客户想走简易/即时办结快速退出，但账上有大额增值税留抵、资产负债未清。\n教训：商贸“一进一出”通常销项大于进项，大额留抵反常、是即时办结的核心卡点；长期零申报还会致纳税信用降级。\n我们的做法：先让会计梳理留抵形成原因并评估解释合理性，清理存货/固资/往来款，工商减资将认缴 500 万降至实缴 100 万，满足即时办结条件后容缺办理、即时取得清税证明。'
    },
    {
      keys: ['继承', '遗产', '过户', '房产', '不动产', '赠与', '遗嘱'],
      title: '父亲留下一套房，姐妹俩纠结“办到自己名下”还是“留母亲名下卖”',
      quote: '想一次办完，别等妈妈百年后再跑一遍',
      points: ['继承过户仅缴万分之五印花税，赠与取得再售按差额 20% 个税', '继承/赠与按政府指导价计税（常低于市价）', '若打算出售，“母亲满五唯一”最省税'],
      detail: '背景：父亲身故，留下一套成本价购买的房产；母亲名下无其他房。\n经过：姐妹想手续最省、税费最低，避免将来母亲百年后重跑一遍继承。\n关键点：父亲的一半份额为遗产；“一次办到姐妹名下”省将来一趟但赠与部分税高、再售个税高；“留母亲名下”若要卖则满五唯一免个税。\n我们的做法：若打算出售，建议姐妹放弃继承、全部归母亲名下（仅几百元印花税），由母亲满五唯一低税出售；先办继承公证，再到不动产登记中心同时办继承与赠与。'
    },
    {
      keys: ['赠与', '子女', '澳洲', '购房', '外汇', '跨境', '汇款', '传承'],
      title: '母亲想每年汇钱给澳洲女儿买房，怕被当成来路不明的钱',
      quote: '就是给女儿买房的钱，怎么证明？',
      points: ['直系赠与澳洲免税，关键在能证明“系直系赠与”', '律师赠与函 + 亲子关系公证 + 银行闭环流水“三件套”', '受中国每人每年 5 万美元购汇额度约束'],
      detail: '背景：离异母亲拟每年用 5 万美元购汇额度向澳洲 PR 女儿赠与购房款。\n经过：两张卡额度不够想借他人卡汇出，担心澳洲反洗钱/CRS 把赠与款当收入征税。\n关键点：澳洲对父母给子女的直系赠与不征税，但要能证明系直系赠与；给移民局的旧材料税局不认。\n我们的做法：备“律师赠与函 + 亲子关系公证（中英双语）+ 银行闭环流水”三件套；借他人卡无妨但须留完整资金路径；中澳各立一份遗嘱做传承衔接。'
    },
    {
      keys: ['跨境电商', '电商', '出口', '亚马逊', '9810', '0110', '赛维', '退税'],
      title: '跨境电商在两种出口模式间反复横跳，还叠着无票、社保一堆隐患',
      quote: '按这个报关、按那个退税，会不会哪天被翻旧账？',
      points: ['9810 vs 0110 须先与主管税局明确执行口径', '混合申报(9810报关+0110退税)有被追溯风险', '“赛维模式”香港汇总+国内集中报关，香港实体要有实质'],
      detail: '背景：跨境电商企业在 9810 与 0110 出口模式间取舍，另有食堂无票、社保、高企、主体混同多处风险。\n经过：当地对 9810 实操熟悉度低、系统支撑不足，客户混合申报心里没底。\n教训：混合申报未来可能被追溯；食堂无票难列支；高企研发费用归集不实会被查补税。\n我们的做法：出口模式先与主管税局确认口径再统一申报，倾向“赛维模式”（香港汇总收入+国内集中报关，香港实体须有人员/场地/实质）；食堂零星采购凭收据入账、社保逐步夯实、研发费用严格归集。'
    }
  ];
...
6a. 【KYC 五维深挖方法（信息质量闸门，务必遵守）】按下列标准判断每一维是"够/部分/空"，只追问"部分/空"的维度：
   · 主体：需 身份角色 + 大致年龄段/人生阶段 + 家庭结构（有无配偶/子女/二代）三要素齐全才算够；
   · 行业：需 细分赛道 + 该行业当前周期/景气度/风险特征，只说"做实业/搞投资"算不够；
   · 事件：需 至少一个明确的已发生或计划中的关键事件 + 大致时间线/触发点，只说方向没时间算不够；
   · 资产：需 主要资产类型 + 规模量级 + 结构（境内/跨境、流动/非流动、个人/企业）；
   · 约束：能由"主体+事件"专业推断即可算够（可标为推断），不必强行追问客户。
6b. 【追问优先级】主体 ≈ 行业 ≈ 事件 > 资产 > 约束（前三者缺失最伤下游）。同一维度最多追 2 次，不要在单一维度上钻太深。
6c. 【每个 suggested_question 必须自带一个贴合本客户行业/身份的填写示例】格式如"……？（比如：……）"，示例要具体、可照着回答，且随客户已透露的行业/身份定制，绝不用无关的通用模板。例：客户是制造业实控人时，资产追问示例应贴合制造业股权与厂房，而非套用地产或互联网的例子。
6d. 绝不臆造：不替客户编造资产数字、身份、事件；缺失就追问或按第14条推断约束，不要脑补填空。追问口吻自然、像顾问聊天，不要暴露"KYC 检测/五维打分"等内部机制。
...
JSON 结构：
{
  ...
  function nextProfileGap(session) {
    ...
    const key = PROFILE_GAP_PRIORITY.find((k) => gaps.has(k) && !asked.has(k));
    if (!key) return null;
